...
class TRECS(transformer.TransformerPrimitiveBase[Inputs, Outputs, TRECSHyperparams]):
    ...
    metadata = metadata_module.PrimitiveMetadata({
        ...
        'source': {
            ...
            'uris': [
                ...
                'https://github.com/dvdmjohnson/d3m_michigan_primitives/blob/master/spider/preprocessing/trecs/trecs.py',
    ...
    def __init__(self, *, hyperparams: TRECSHyperparams, random_seed: int = 0, docker_containers: typing.Dict[str, base.DockerContainer] = None) -> None:
        """
        Primitive for resampling video frames according T-RECS methods.
        """
    ...
    def _input_handler(self, data):
        """
        Check input type and loads video if necessary

        Arugments:
            :data: string file location of a video, or np.array of a video

        Returns:
            :output_vid: 4D np.array of the video to be resampled

        """
        # If data is a path to a video, load the video
        if isinstance(data, str):
            if not os.path.isfile(data):
                raise IOError('Input video file does not exist.')
            else:
                output_vid = self._load_video(data)
        
        # If data is already a numpy array pass it through
        elif isinstance(data, np.ndarray):
            output_vid = data

        else:
            raise TypeError("The input to produce must either be a path to a video or an np array containing a video")
    ...
    @classmethod
    def can_accept(cls, *, method_name: str, arguments: typing.Dict[str, typing.Union[metadata_module.Metadata, type]], hyperparams: TRECSHyperparams) -> typing.Optional[metadata_module.DataMetadata]:
