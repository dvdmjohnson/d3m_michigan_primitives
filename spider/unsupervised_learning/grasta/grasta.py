...
class GRASTAHyperparams(hyperparams.Hyperparams):
    ...
    subsample = hyperparams.Bounded[float](lower=0.01,
      upper=1,
      default=1,
      semantic_types=[
...
class _OPTIONS(object):
    def __init__(self, rank, subsampling=1, admm_max_itr=20, admm_min_itr=20, max_level=20, max_mu=15, min_mu=1,constant_step=0):
...
class GRASTA(unsupervised_learning.UnsupervisedLearnerPrimitiveBase[Inputs, Outputs, GRASTAParams, GRASTAHyperparams]):
    ...
    self._random_state = np.random.RandomState(random_seed)
        ...
        def set_training_data(self, *, inputs: Inputs) -> None:
            self._X = inputs
            logger.warn('Set training data inputs sizes: ' + str(inputs.shape))
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> base.CallResult[None]:
        ...
        self._grastaOPTIONS = _OPTIONS(self._rank, self._subsample, self._admm_max_iter,
         self._admm_min_iter,
        ...
        for i in range(0, max_cycles):
            ...
            for j in range(0, train_size):
                ...
                if(self._grastaOPTIONS.subsampling < 1):
                    _xidx = self._random_state.choice(d, int(np.ceil(self._grastaOPTIONS.subsampling * d)),replace=False)
                else:
                    _xidx = np.where(~np.isnan(vec))[0]
